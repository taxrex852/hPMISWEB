<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="3101.aspx.vb" Inherits="hPMISWEB.HSM_Produce" ContentType="text/html" ResponseEncoding="UTF-8" %>
    ...
    <form id="form1" runat="server">
        ...
        <div class="container-fluid main-content px-4">
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:PMISConnectionString %>"
                SelectCommand="WITH MonthlyCoil AS ( SELECT DATEADD(month, DATEDIFF(month, 0, product_date), 0) AS prod_month, SUM(CASE WHEN reject_reason = 0 THEN coil_weight ELSE 0 END) / 1000.0 AS PA_wgt, SUM(CASE WHEN reject_reason = 0 THEN slab_weight ELSE 0 END) / 1000.0 AS PY_slab_wgt, SUM(CASE WHEN reject_reason &lt;&gt; 0 THEN slab_weight ELSE 0 END) / 1000.0 AS MR_wgt FROM [HPMIS].[dbo].[h_pmis_coil_info] WHERE product_date BETWEEN DATEADD(year, -1, GETDATE()) AND GETDATE() GROUP BY DATEADD(month, DATEDIFF(month, 0, product_date), 0) ), MonthlyWhqh AS ( SELECT DATEADD(month, DATEDIFF(month, 0, process_date), 0) AS prod_month, SUM(coil_wm) / 1000.0 AS coil_wm FROM [HPMIS].[dbo].[h_pmis_whqh] WHERE final_dis NOT IN ('1', '2', 'H') AND process_date BETWEEN DATEADD(year, -1, GETDATE()) AND GETDATE() GROUP BY DATEADD(month, DATEDIFF(month, 0, process_date), 0) ), MonthlySi01 AS ( SELECT DATEADD(month, DATEDIFF(month, 0, process_date), 0) AS prod_month, SUM(acci_delay_time + roll_delay_time + shutdown_time + others_delay_time) AS total_delay, SUM(shutdown_time) AS total_shutdown FROM [HPMIS].[dbo].[h_pmis_si01] WHERE line_id = 1 AND process_date BETWEEN DATEADD(year, -1, GETDATE()) AND GETDATE() GROUP BY DATEADD(month, DATEDIFF(month, 0, process_date), 0) ) SELECT CONVERT(varchar(7), c.prod_month, 111) AS product_date, ISNULL(ROUND(c.PA_wgt, 2), 0) AS PA, ISNULL(ROUND((c.PA_wgt * 100.0) / NULLIF(c.PY_slab_wgt, 0), 2), 0) AS PY, ISNULL(ROUND(((c.PA_wgt - ISNULL(w.coil_wm, 0)) * 100.0) / NULLIF(c.PY_slab_wgt, 0), 2), 0) AS PO, ISNULL(ROUND((s.total_delay * 100.0) / NULLIF(s.total_shutdown, 0), 2), 100.0) AS [OR], ISNULL(ROUND(c.MR_wgt, 2), 0) AS MR FROM MonthlyCoil c LEFT JOIN MonthlyWhqh w ON c.prod_month = w.prod_month LEFT JOIN MonthlySi01 s ON c.prod_month = s.prod_month ORDER BY c.prod_month">
            </asp:SqlDataSource>

            <div class="card-custom mb-4 mt-2">
                <div class="card-header-custom">
                    <span class="fs-4">📊 生產效益與重量趨勢</span>
